...
module.exports = {
	...
	async execute(interaction) {
		...
		let emoji = interaction.options.getString('reaction');
		if (!emoji) {
			emoji = 'all';
		}
		...
		if (emoji == 'all') {
			jsonData[interaction.user.id][interaction.guild.id] = [];
			// Write the file
		...
		else if (userEmojiList.includes(emoji)) {
			// remove the emoji from the list
			userEmojiList.splice(userEmojiList.indexOf(emoji), 1);
			jsonData[interaction.user.id][interaction.guild.id] = userEmojiList;
		}
		else {
